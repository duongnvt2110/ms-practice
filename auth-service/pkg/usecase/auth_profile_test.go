...
func assertAppError(t *testing.T, err error, expected errorsx.AppError) {
	t.Helper()
	if err == nil {
...
func TestRegister_ErrorMapping(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: errorsx.ErrInternalServerError,
		},
	}
...
func TestRefreshToken_ErrorMapping(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			token:   "",
			wantErr: apperr.ErrRefreshTokenRequired,
		},
		{
