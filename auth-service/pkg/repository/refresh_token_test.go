...
func assertRepoAppError(t *testing.T, err error, expected errorsx.AppError) {
	t.Helper()
	if err == nil {
...
func TestRefreshTokenRepo_GetByToken_NotFound(t *testing.T) {
	...
	assertRepoAppError(t, err, apperr.ErrInvalidRefreshToken)
}
...
func TestRefreshTokenRepo_GetByToken_InternalError(t *testing.T) {
	...
	_, err := repo.GetByToken(context.Background(), "missing")
	assertRepoAppError(t, err, errorsx.ErrInternalServerError)
}
